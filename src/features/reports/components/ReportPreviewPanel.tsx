        ...
        <div className="grid grid-cols-2 gap-4">
            ...
            <div className="rounded-xl border border-[var(--bg-border)] bg-[var(--bg-card)] p-4 flex flex-col gap-3">
                    ...
                    >
                        {reports?.map(r => (
                            <option key={r.id} value={r.id}>
                                {r.name[language]} – {r.dateGenerated}
                            </option>
                        ))}
                    </select>
                </div>
                ...
                <div className="flex gap-3">
                    ...
                    <div className="flex flex-col gap-2 flex-1">
                        <p className="text-[11px] font-semibold text-[var(--text-primary)]">{t.reports.summary}</p>
                        <p className="text-[10px] text-[var(--text-muted)] leading-relaxed">
                            {t.reports.summaryText} {report?.type?.toLowerCase()} {t.reports.summaryText2}
                        </p>
                        <p className="text-[11px] font-semibold text-[var(--text-primary)] mt-1">{t.reports.keyFindings}</p>
                        <ul className="flex flex-col gap-1">
                            {t.reports.keyFindingsList.map((f, i) => (
            ...
            <div className="rounded-xl border border-[var(--bg-border)] bg-[var(--bg-card)] p-4 flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-3">
                    {[
                        { label: t.reports.includeCharts, sub: t.reports.includeChartsub, value: includeCharts, toggle: () => setIncludeCharts(v => !v) },
                        { label: t.reports.includeRaw, sub: t.reports.includeRawSub, value: includeRaw, toggle: () => setIncludeRaw(v => !v) },
                        { label: t.reports.includeBranding, sub: t.reports.includeBrandSub, value: includeBranding, toggle: () => setIncludeBranding(v => !v) },
