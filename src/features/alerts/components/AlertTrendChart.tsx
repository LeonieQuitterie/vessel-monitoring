        ...
        <div className="rounded-xl border border-[var(--bg-border)] bg-[var(--bg-card)] p-4">
            <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-3">
                    {[
                        ...
                        { label: 'Informational', color: 'var(--status-info)' },
                    ].map(({ label, color }) => (
            ...
            <ResponsiveContainer width="100%" height={140}>
                <LineChart data={trendData} margin={{ top: 4, right: 4, left: -24, bottom: 0 }}>
                    ...
                    <Line type="monotone" dataKey="critical" stroke="var(--status-danger)" strokeWidth={1.5} dot={false} name="Critical" isAnimationActive={false} />
                    <Line type="monotone" dataKey="warning" stroke="var(--status-warn)" strokeWidth={1.5} dot={false} name="Warning" isAnimationActive={false} />
                    <Line type="monotone" dataKey="info" stroke="var(--status-info)" strokeWidth={1.5} dot={false} name="Informational" isAnimationActive={false} />
                </LineChart>
            </ResponsiveContainer>
        </div>
    )
